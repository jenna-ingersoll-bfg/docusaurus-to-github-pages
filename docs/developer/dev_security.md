...
# Sending Messages Manually with the Leanplum API and Postman

Leanplum has different delivery methods to choose from when sending messages to the player. Examples include in-app messages, push notifications, gift inboxes, e-mails, webhooks, and more. 

While all of these can be implemented programmatically, you may want to manually deliver a message to the user. Manually sending messages with Postman can be useful particularly when sending messages to a single device for testing, without the need of an event trigger or when you are concerned about message limits. 

:::warning

While most types of messages support manual delivery, you cannot send in-app messages manually with Postman.
...
To use the Postman API and web app, you must first create a free account. In addition, you may wish to download and install the Postman desktop app.

:::

Below is a Postman collection with a set of requests to get you started. Copy this JSON code into a text file, then save it. Import the saved file into your Postman workspace using the **Import** button.
...
Once the collection has been added, variables like ``testAppKey`` might need to be updated to reflect the environment and app you are trying to use. You can find the keys in **Leanplum/Development/App Settings** of the [Leanplum API Copy](https://www.leanplum.com/dashboard2/5098074856816640/settings/api-keys).
...
Instead of using Postman, Leanplum also provides a [webpage to send requests](https://docs.leanplum.com/reference/api-introduction) that also lists descriptions for all the different possible requests. The benefit of using Postman allows you to save values; Leanplum’s webpage will not allow you to save your values.

## Setting up the Message in Leanplum 

Creating a message with the delivery is simple. If you need more information on how to create messages in Leanplum, refer to the following documents: In-App Messaging with Leanplum, Push Notifications with Leanplum and App Inbox with Leanplum.

1. Create a new message in Leanplum.
2. Scroll down to Delivery.
3. Select Manual from the drop-down.

If it is not available, then the message does not support manual delivery.


## Sending Request on using Leanplum API 

Once the message on Leanplum is configured and set to Manual, use the API call ``sendMessage`` to trigger the message. This will send the message to the device specified. Make sure to edit the ``userId`` and ``messageId`` params to the desired values. 

The response will let you know if you have successfully sent the message.

If you created a campaign that is available for Manual Delivery, you can use ``startCampaign`` to trigger the messages. Note that if you use ``startCampaign``, the variables will be in the **Body** section instead of **Params**.
